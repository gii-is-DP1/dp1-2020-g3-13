...
public class PeticionController {
    ...
    public String crearOrganizacionByPeticion(@PathVariable("peticionid") Integer peticionid, ModelMap modelMap) {
        ...
        usuarioService.saveUser(newUsuario);
        ...
        peticionServ.deletePeticion(peti.get());
        envioEmailService.sendEmailSimple(newOrg, "Petición de Organización",
                "Hola ! Desde Qplan queremos darte la bienvenida después de aceptar tu petición\nYa puedes iniciar sesión en la web con las siguientes credenciales\nUsuario: "
                        + newOrg.getUsuario().getNombreUsuario() + "\nContraseña: " + newOrg.getUsuario().getPassword()
                        + "\n¡Bienvenid@!");
        // sendEmail(newOrg, "Petición QPlan aceptada", "<html><body><h3>Hola,
        // "+newOrg.getUsuario().getPassword() +"welcome to the Chipping Sodbury
        // On-the-Hill message boards!</h3>");
