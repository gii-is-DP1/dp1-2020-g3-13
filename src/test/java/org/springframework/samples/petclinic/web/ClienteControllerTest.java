...
public class ClienteControllerTest {
    private static final String VIEWS_CONSULTA_CREATE_OR_UPDATE_FORM = "clientes/createOrUpdateClienteForm";
    @Autowired
 ...
 //Prueba a hacer la petición GET formulario de consultas
    @WithMockUser
    @Test 
    public void testFormularioCrearCliente() throws Exception {
        mockMvc.perform(get("/clientes/new")).andExpect(status().isOk())
        .andExpect(view().name(VIEWS_CONSULTA_CREATE_OR_UPDATE_FORM)).andExpect(model().attributeExists("cliente"));
    }
}
